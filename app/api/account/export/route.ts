...
export async function GET(request: Request) {
  ...
  const [
    ...
    runIds.length > 0
      ...
      : Promise.resolve([]),
    runIds.length > 0
      ? batchIn<Record<string, unknown>>(admin, 'filter_decisions', 'id, fsn_result_id, search_run_id, decision, rationale, confidence, model_used, created_at', 'search_run_id', runIds)
      : Promise.resolve([]),
    profileIds.length > 0
