...
const CreateProfileSchema = z.object({
  ...
  strategy_doc_paths: z.array(z.string().max(500).refine((s) => { const d = decodeURIComponent(s); return !d.includes('..') && !d.startsWith('/') && !d.includes('\0') && !s.includes('\0') }, 'Invalid file path')).max(5).default([]),
})
